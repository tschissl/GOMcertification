...
df <- df[!1,]
#write.table(df, "df2.txt", row.names=FALSE) # needed only the first time
...
write.table(df, "df2.txt", row.names=FALSE, append=TRUE, col.names=FALSE)
  }
```

```{r}
df2 <- read.table("~/GitHub/GOMcertification/df2.txt", header=T, quote="\"")
final <- ddply(df2, c("date","RM","measurand","based.on", "unit"), numcolwise(meanGOM))
```
#### final
